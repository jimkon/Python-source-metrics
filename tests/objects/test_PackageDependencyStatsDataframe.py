...
class TestPackageDependencyStatsDataframe(unittest.TestCase):
    ...
    @mock.patch.object(ImportsEnrichedDataframe, '__new__')
    def test_cyclic_imports(self, mock_imports):
        test_df = pd.DataFrame({
            'package': ['a1', 'a1', 'a1', 'a2', 'a2', 'a2', 'a2', 'a2', 'a3'],
            'import_package': ['ext_1', 'ext_1', 'ext_2', 'a1', 'a2', 'ext_2', 'a2', 'ext_3', 'a3'],
        })
        expected_df = pd.DataFrame({
            'package': ['a2', 'a3'],
            'imports_itself': [2, 1],
        ...
        obj = PackageDependencyStatsDataframe()
        obj._imports_df = test_df
        pkg_deps = obj.produce_cyclic_imports()
        pd.testing.assert_frame_equal(pkg_deps, expected_df)
